  ...
  axiosUtil
    ...
    .then((response) => {
      if (response.data.isDeleted) {
        comments.value[comment_index].likes.pop();
      } else {
...
const props = defineProps(["scrollPos", "commentData", "postId"]);
...
onMounted(async () => {
  ...
  setTimeout(() => {
    lottie.value.forEach((element, index) => {
      //oturum açan kullanıcının idsini includes içine yaz
      if (
        comments.value[index].likes.includes(
          localStorage.getItem("currentUser")
        )
      ) {
        isPlayingFirstPart.value[index] = false;
        element.playSegments([40, 41], true);
      }
    });
  }, 10);
});
