  ...
  >
    ...
    <div class="h-full col-span-10 lg:col-span-3 dark:bg-gray-900 bg-gray-100 dark:text-white p-5 flex flex-wrap content-start">
      <div class="w-full text-2xl font-bold flex justify-between">
        <div>Yorumlar(1)</div>
        <div>
          <font-awesome-icon :icon="['fas', 'xmark']" class="cursor-pointer" @click="commentClose" />
        </div>
      </div>
      <!--Comment write-->
      <div class="w-full mt-5 rounded-lg shadow-xl flex flex-wrap content-start border border-b-2 bg-white">
        ...
        <div class="w-full p-3 flex flex-wrap content-start">
          <textarea v-model="commentText" class="w-full bg-inherit p-2 resize-none outline-none" placeholder="Düşüncelerini Yaz"></textarea>
              ...
              <div class="flex items-center">
                ...
                >
                  <div class="flex w-full text-center items-center">
                    <img class="w-8 h-8 rounded-full mr-2" :src="comment.author_data.avatar" alt="User Photo" />
                    <div>
                      <h2 class="text-lg font-semibold">
...
onBeforeMount(() => {
  ...
  for (let comment of comments.value) {
    let author = {}
    axiosUtil.get(`/user/fetch/${comment.author_id}?fields=avatar,name,email`).then(response => {
